...
function parse() {
  ...
  var isDecode = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  ...
  return urlParams ? JSON.parse('{"' + urlParams.replace(/&/g, '","').replace(/=/g, '":"') + '"}', transData) : {};
}
/**
 * 传入要转换的对象，返回url字符串
...
function toUrl(urlObj) {
  ...
  return Object.keys(urlObj).reduce(function (total, item, index) {
    total += "".concat(index ? '&' : '').concat(item, "=").concat(urlObj[item]);
    return total;
  }, '?');
}
/**
...
function getQueryString(key, isDecode) {
  ...
  var matchArr = window.location.search.substr(1).match(regExp);
